    ...
    tests: {
      webformDate: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          validator.form();
          // Check for the "Date" error.
          equal($('label[for=webform-component-date_group].error:visible').length, 0, Drupal.t('Error label not found for "Date"'));
        }
      },
      webformEmail: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the email error.
          equal($('label[for=edit-submitted-email].error:visible').length, 0, Drupal.t('Error label not found for "Email"'));
        }
      },
      webformNumber: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-number].error:visible').length, 0, Drupal.t('Error label not found for "Number"'));
        }
      },
      webformSingleSelect: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the email error.
          equal($('label[for=edit-submitted--single-select].error:visible').length, 0, Drupal.t('Error label not found for "Single select"'));
        }
      },
      webformMultipleSelect: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-multiple-select].error:visible').length, 1, Drupal.t('Error label found for "Multiple select"'));
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-multiple-select].error:visible').length, 0, Drupal.t('Error label not found for "Multiple select"'));
        }
      },
      webformRadios: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for="submitted[radio_buttons]"].error:visible').length, 0, Drupal.t('Error label not found for "Radio Buttons"'));
        }
      },
      webformCheckboxes: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for="submitted[checkboxes]_group"].error:visible').length, 0, Drupal.t('Error label not found for "Checkboxes"'));
        }
      },
      webformTextarea: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-textarea].error:visible').length, 0, Drupal.t('Error label not found for "Textarea"'));
        }
      },
      webformTextfield: function($, Drupal, window, document, undefined) {
        return function() {
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-textfield].error:visible').length, 0, Drupal.t('Error label not found for "Textfield"'));
        }
      },
      webformTime: function($, Drupal, window, document, undefine) {
        return function() {
          expect(8);
          // Validate the empty form.
          validator.form();
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-time-hour].error:visible').length, 1, Drupal.t('Error label found for "Time (Hour)"'));
          equal($('label[for=edit-submitted-time-minute].error:visible').length, 1, Drupal.t('Error label found for "Time (Minute)"'));
          ...
          // Check for the error.
          equal($('label[for=edit-submitted-time-hour].error:visible').length, 0, Drupal.t('Error label not found for "Time (Hour)"'));
          equal($('label[for=edit-submitted-time-minute].error:visible').length, 1, Drupal.t('Error label found for "Time (Minute)"'));
          ...
          // Check for the "Date" error.
          equal($('label[for=edit-submitted-time-hour].error:visible').length, 1, Drupal.t('Error label found for "Time (Hour)"'));
          equal($('label[for=edit-submitted-time-minute].error:visible').length, 0, Drupal.t('Error label not found for "Time (Minute)"'));
